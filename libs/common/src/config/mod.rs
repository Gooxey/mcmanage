...
use notify::{RecommendedWatcher, Event, Watcher, RecursiveMode, ReadDirectoryChangesWatcher};
...
impl Config {
    ...
    ///     1. Create a static holding the application's [`Config`].
    ///     2. Create a static holding the [`watcher`](RecommendedWatcher) responsible for updating every field of the [`Config`] on a change to the file at `config/config.toml`. \
    /// \
    ...
    pub async fn new() -> Arc<Mutex<Self>> {
        ...
        lazy_static! {
            static ref CONFIG: AsyncOnce<Arc<Mutex<Config>>> = AsyncOnce::new(async {
                Arc::new(Mutex::new(
                    load_config().await
                ))
            });
        ...
        info!("Main", "Starting the watcher for the applications config...");
        lazy_static! {
            static ref CONFIG_WATCHER: Mutex<ReadDirectoryChangesWatcher> =
                RecommendedWatcher::new(move |result: Result<Event, notify::Error>| {
                    let event = result.unwrap_or_else(|erro| panic!("An error occurred while watching the file at 'config/config.toml'. Error: {erro}"));
                    ...
                    .unwrap_or_else(|erro| panic!("An error occurred while creating a watcher for the file at 'config/config.toml'. Error: {erro}"))
                    .into();
        }

        if !Path::exists(Path::new("config/config.toml")) {
            warn!("Main", "Could not find a the file at 'config/config.toml'. A default config file will be generated.");
            replace_with_valid_file("config", "config", CONFIG_DEFAULT).await;
        }

        CONFIG_WATCHER.lock().await
            .watch(Path::new("config/config.toml"), RecursiveMode::NonRecursive)
            .unwrap_or_else(|erro| panic!("An error occurred while starting a watcher for the file at 'config/config.toml'. Error: {erro}"));
    ...
    pub async fn communicator_port(config: &Arc<Mutex<Self>>) -> u16 {
        config.lock().await.communicator_port
    }
